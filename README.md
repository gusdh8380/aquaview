...
**수처리 공정 모니터링 시스템** — 실시간 센서 데이터를 2D 차트와 3D 시각화로 모니터링하는 풀스택 대시보드

## 🎯 프로젝트 개요

AquaView는 수처리 시설의 핵심 센서(pH, 탁도, 유량, 수온)를 실시간으로 모니터링하는 웹 기반 대시보드입니다. FastAPI 백엔드가 센서 데이터를 시뮬레이션하고, React 프론트엔드가 차트와 경보를 표시하며, Unity WebGL이 3D 공정 뷰를 제공합니다.
...
```
React (usePolling 3초)
  └─ fetchSensors() → JSON 수신
      └─ postMessage({ type: "SENSOR_UPDATE", sensors })
          └─ Unity iframe 수신
              └─ SensorDataReceiver.cs → 이벤트 브로드캐스트
                  ├─ TankController: 수위 변화 + 물 색상
                  ├─ PipeController: 파이프 색상
                  └─ SensorLabel: 텍스트 값 표시
```
